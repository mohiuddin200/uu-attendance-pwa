...
export const openSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (routine.batch !== profile.batch || routine.section !== profile.section) {
    ...
    for (const session of openSessions) {
      if (session.closesAt > now) {
        throw new Error('An attendance session is already open for this section.')
      }
...
export const submit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (session.batch !== profile.batch || session.section !== profile.section) {
...
export const manualMarkPresent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!isUniversityEmail(email)) {
      throw new Error(`Manual attendance requires an @${UNIVERSITY_DOMAIN} email.`)
    }
    ...
    if (
      matchedProfile &&
      (matchedProfile.batch !== session.batch || matchedProfile.section !== session.section)
    ) {
